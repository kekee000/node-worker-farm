...
Farm.prototype.onExit = async function (childId) {
...
Farm.prototype.startChild = async function () {
  ...
  let forked = await fork(this.path, this.options.workerOptions)
    , id     = this.childId
    , c      = {
          send        : forked.send
        , child       : forked.child
...
Farm.prototype.processQueue = async function () {
  ...
  for (cka = this.childKeys(); i < cka.length; i++) {
    ...
    if (this.children[childId].activeCalls < this.options.maxConcurrentCallsPerWorker
      ...
      console.log('this.callQueue.length', this.callQueue.length);
      if (!this.callQueue.length) {
        console.log('this.callQueue.length222', this.callQueue.length);
        return this.ending && this.end()
      }
    } /*else {
      console.log(
