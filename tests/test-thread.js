...
tape('simple, exports=function test', async function (t) {
    ...
    let child = await workerFarm({enableWorkerThreads: true}, childPath)
    child(0, function (err, pid, rnd) {
        t.ok(pid > 0, 'pid makes sense 1')
...
tape('simple, exports.fn test', async function (t) {
    ...
    let child = await workerFarm({enableWorkerThreads: true}, childPath, ['run0'])
    child.run0(function (err, pid, rnd) {
        t.ok(pid > 0, 'pid makes sense')
        t.ok(rnd >= 0 && rnd < 1, 'rnd result makes sense')
    })
...
tape('on child', async function (t) {
    ...
    let child = await workerFarm({
        ...
        onChild: function (subprocess) {
            t.ok(!!subprocess.threadId, 'threadId');
            childPid = subprocess.threadId
            t.ok(childPid >= 0, 'threadId > 0');
        },
        maxConcurrentWorkers: 1
    }, childPath);
...
tape('single worker', async function (t) {
    ...
    let child = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 1 }, childPath)
        , pids = []
        , i = 10
...
tape('two workers', async function (t) {
    ...
    let child = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 2 }, childPath)
        , pids = []
        , i = 10
    ...
    while (i--) {
        child(0, function (err, pid) {
            pids.push(pid)
            if (pids.length == 10) {
                t.equal(2, uniq(pids).length, 'only two thread (by pid)')
            } else if (pids.length > 10)
                t.fail('too many callbacks!')
        })
    }
...
tape('auto start workers', async function (t) {
    let child = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 3, autoStart: true }, childPath, ['uptime'])
        , pids = []
        , count = 5
        , i = count
        , delay = 250
...
tape('durability', async function (t) {
    ...
    let child = await workerFarm({enableWorkerThreads: true, maxConcurrentWorkers: 2 }, childPath, ['killable'])
        , ids = []
        , pids = []
        , count = 20
        , i = count
...
tape('simple, end callback', async function (t) {
    ...
    let child = await workerFarm({enableWorkerThreads: true}, childPath)
    child(0, function (err, pid, rnd) {
        t.ok(pid > 0, 'pid makes sense ' + pid + ' vs ' + process.pid)
        t.ok(rnd >= 0 && rnd < 1, 'rnd result makes sense')
    })
...
tape('test error passing', async function (t) {
    ...
    let child = await workerFarm({enableWorkerThreads: true}, childPath, ['err'])
...
tape('test maxConcurrentCalls', async function (t) {
    ...
    let child = await workerFarm({ enableWorkerThreads: true, maxConcurrentCalls: 5 }, childPath)
...
tape('test maxConcurrentCalls + queue', async function (t) {
    ...
    let child = await workerFarm({ enableWorkerThreads: true, maxConcurrentCalls: 4, maxConcurrentWorkers: 2, maxConcurrentCallsPerWorker: 1 }, childPath)
...
tape('test timeout kill', async function (t) {
    ...
    let child = await workerFarm({ enableWorkerThreads: true, maxCallTime: 250, maxConcurrentWorkers: 1 }, childPath, ['block'])
...
tape('test max retries after process terminate', async function (t) {
    ...
    let child1 = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 1, maxRetries: 5 }, childPath, ['stubborn'])
    ...
    let child2 = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 1, maxRetries: 3 }, childPath, ['stubborn'])
...
tape('custom arguments can be passed to "fork"', async function (t) {
    ...
    let workerOptions = {
        ...
        , child = await workerFarm({ enableWorkerThreads: true, maxConcurrentWorkers: 1, maxRetries: 5, workerOptions: workerOptions }, childPath, ['args'])

    child.args(function (err, result) {
        t.equal(result.execArgv[0], '--no-addons', 'flags passed (overridden default)')
    })
...
tape('NO SUCH METHOD', async function (t) {
    ...
    let child = await workerFarm({
        enableWorkerThreads: true,
        maxConcurrentWorkers: 1,
        onChild(child, forked) {
            t.ok(child.threadId, 'has thread id');
            forked.send({});
        }
